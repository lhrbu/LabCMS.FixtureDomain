    ...
    public class CheckoutRecordsController : ControllerBase
    {
        ...
        public IAsyncEnumerable<CheckoutRecord> GetCheckoutRecordsHistory()
        {
            ...
            return _repository.FixtureCheckoutRecords.Where(item => item.ApplicantUserId == rolePayload.UserId)
                ...
                //.Where(item=>item.Fixture!.InFixtureRoom)
        ...
        public async ValueTask<ActionResult<CheckoutRecord>> InitApplicationAsync(CheckoutRecordPayload checkoutRecordInClient)
        {
            ...
            // { return NotFound($"Fixture {checkoutRecordInClient.FixtureNo} is not in fixture room!");}
        ...
        {
            RolePayload rolePayload = (HttpContext.Items[nameof(RolePayload)] as RolePayload)!;
            CheckoutRecord checkoutRecord = (HttpContext.Items[id] as CheckoutRecord)!;
        ...
        [RolePayloadRequired]
        [PermissionPolicy(typeof(TestFieldResponseAuthorizePolicy))]
        ...
        //public async ValueTask<CheckoutRecord> FixtureRoomApproveAsync(int id)
        //{
        //    CheckoutRecord checkoutRecord = (HttpContext.Items[id] as CheckoutRecord)!;
        //    RolePayload rolePayload = (HttpContext.Items[nameof(RolePayload)] as RolePayload)!;
        ...
        //public async ValueTask<CheckoutRecord> EndAsync(int id)
        //{
        //    CheckoutRecord checkoutRecord = (HttpContext.Items[id] as CheckoutRecord)!;
        //    checkoutRecord.Status = CheckRecordStatus.End;
        //    checkoutRecord.CheckoutDate = DateTimeOffset.Now;
        //    _repository.Update(checkoutRecord);
        //    await _repository.Entry(checkoutRecord).Reference(item=>item.Fixture).LoadAsync();
        //    await _repository.Entry(checkoutRecord).Reference(item=>item.Applicant).LoadAsync();
        //    string date = DateTimeOffset.Now.LocalDateTime.ToString("yyyy-MM-dd HH:mm");
        //    checkoutRecord.Fixture!.StorageInformation = $"[ {date} ]: Check out by {checkoutRecord.ApplicantUserId}";
        //    await _repository.SaveChangesAsync();
        //    return checkoutRecord;
        //}
        #endregion
